...
public class WorkoutController {
    ...
    @GetMapping
    public List<WorkoutResponseDTO> list() {
        return workoutService.listAll();
    }

    @GetMapping("/{id}")
    public WorkoutResponseDTO findById(@PathVariable Long id) {
        return workoutService.findById(id);
    }

    @GetMapping("/user/{userId}")
    public List<WorkoutResponseDTO> findAllByUserId(@PathVariable Long userId, @AuthenticationPrincipal User loggedUser) {
